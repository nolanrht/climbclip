...
const PILE = Array.from({length:140},(_,i)=>{
  const a=((i*1234567+89)%997)/997, b=((i*7654321+123)%1009)/1009
  const c=((i*3456789+456)%1013)/1013, d=((i*9876543+789)%1019)/1019
  const ly=b<0.40?0:b<0.68?1:b<0.85?2:3
  return {x:a*1200, y:800-ly*16-c*13, rot:d*360, rx:3.5+a*6, ry:(2.5+a*5)*(0.44+b*0.34), color:c>0.52?"#f5c2c7":"#f5f0e8", op:0.40+c*0.42}
})

const SakuraBg = () => {

  const petals = [
    {l:8,  a:"fallA", d:"15s", dl:"0s",  c:"#f5c2c7", r:"20deg",  s:7},
    {l:22, a:"fallC", d:"18s", dl:"5s",  c:"#f5f0e8", r:"-30deg", s:8},
    {l:45, a:"fallB", d:"14s", dl:"9s",  c:"#f5c2c7", r:"45deg",  s:6},
    {l:63, a:"fallA", d:"16s", dl:"3s",  c:"#f5f0e8", r:"-18deg", s:9},
    {l:78, a:"fallC", d:"13s", dl:"7s",  c:"#f5c2c7", r:"35deg",  s:7},
    {l:90, a:"fallB", d:"17s", dl:"11s", c:"#f5f0e8", r:"-42deg", s:8},
  ]

  return (
    <>
      <style>{`
        @keyframes fallA{0%{transform:translateY(-20px) rotate(0deg);opacity:0}10%{opacity:.55}90%{opacity:.30}100%{transform:translateY(calc(100vh + 40px)) translateX(55px) rotate(380deg);opacity:0}}
        @keyframes fallB{0%{transform:translateY(-20px) rotate(20deg);opacity:0}10%{opacity:.50}90%{opacity:.25}100%{transform:translateY(calc(100vh + 40px)) translateX(-50px) rotate(-360deg);opacity:0}}
        @keyframes fallC{0%{transform:translateY(-20px) rotate(-15deg);opacity:0}10%{opacity:.55}55%{transform:translateY(52vh) translateX(22px) rotate(195deg)}90%{opacity:.28}100%{transform:translateY(calc(100vh + 40px)) translateX(-10px) rotate(405deg);opacity:0}}
      `}</style>
      ...
      <svg style={{position:"fixed",inset:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:0}}
        ...
        <g stroke="#f5f0e8" fill="none" strokeLinecap="round">
          <path strokeWidth="2.0" opacity="0.36" d="M 0,5 C 75,22 182,57 298,108"/>
          <path strokeWidth="2.0" opacity="0.30" d="M 5,0 C 40,68 55,152 44,228"/>
          <path strokeWidth="1.6" opacity="0.28" d="M 2,0 C 100,16 238,37 390,68"/>
          <path strokeWidth="1.1" opacity="0.24" d="M 182,57 C 190,30 208,13 232,4"/>
          <path strokeWidth="1.1" opacity="0.22" d="M 298,108 C 318,74 346,57 380,52"/>
          <path strokeWidth="1.1" opacity="0.22" d="M 390,68 C 403,42 422,29 452,24"/>
          <path strokeWidth="1.0" opacity="0.20" d="M 44,155 C 16,142 1,122 -7,100"/>
          <path strokeWidth="1.0" opacity="0.18" d="M 44,228 C 20,212 6,192 -2,168"/>
          <path strokeWidth="1.0" opacity="0.22" d="M 238,37 C 246,12 262,0 285,-6"/>
          <g fill="#f5c2c7" stroke="none" opacity="0.54">
            <circle cx="232" cy="3"   r="4"/><circle cx="224" cy="-3"  r="3"/><circle cx="240" cy="-2"  r="3.5"/>
            <circle cx="380" cy="50"  r="4"/><circle cx="388" cy="44"  r="3"/><circle cx="373" cy="45"  r="3"/>
            <circle cx="452" cy="22"  r="3.5"/><circle cx="460" cy="16" r="2.5"/><circle cx="444" cy="17" r="2.5"/>
            <circle cx="-7"  cy="98"  r="3.5"/><circle cx="-14" cy="92" r="2.5"/>
            <circle cx="-2"  cy="166" r="3"/><circle cx="-9"  cy="161" r="2.5"/>
            <circle cx="285" cy="-7"  r="3.5"/><circle cx="293" cy="-13" r="2.5"/>
          </g>
          <g fill="#f5f0e8" stroke="none" opacity="0.22">
            <circle cx="228" cy="0"   r="2"/><circle cx="376" cy="47"  r="1.5"/>
            <circle cx="289" cy="-10" r="1.5"/><circle cx="456" cy="19" r="1.5"/>
          </g>
        </g>

        {/* Top-right corner branches (mirror) */}
        <g stroke="#f5f0e8" fill="none" strokeLinecap="round">
          <path strokeWidth="2.0" opacity="0.36" d="M 1200,5 C 1125,22 1018,57 902,108"/>
          <path strokeWidth="2.0" opacity="0.30" d="M 1195,0 C 1160,68 1145,152 1156,228"/>
          <path strokeWidth="1.6" opacity="0.28" d="M 1198,0 C 1100,16 962,37 810,68"/>
          <path strokeWidth="1.1" opacity="0.24" d="M 1018,57 C 1010,30 992,13 968,4"/>
          <path strokeWidth="1.1" opacity="0.22" d="M 902,108 C 882,74 854,57 820,52"/>
          <path strokeWidth="1.1" opacity="0.22" d="M 810,68 C 797,42 778,29 748,24"/>
          <path strokeWidth="1.0" opacity="0.20" d="M 1156,155 C 1184,142 1199,122 1207,100"/>
          <path strokeWidth="1.0" opacity="0.18" d="M 1156,228 C 1180,212 1194,192 1202,168"/>
          <path strokeWidth="1.0" opacity="0.22" d="M 962,37 C 954,12 938,0 915,-6"/>
          <g fill="#f5c2c7" stroke="none" opacity="0.54">
            <circle cx="968"  cy="3"   r="4"/><circle cx="976"  cy="-3"  r="3"/><circle cx="960"  cy="-2"  r="3.5"/>
            <circle cx="820"  cy="50"  r="4"/><circle cx="812"  cy="44"  r="3"/><circle cx="827"  cy="45"  r="3"/>
            <circle cx="748"  cy="22"  r="3.5"/><circle cx="740" cy="16"  r="2.5"/><circle cx="756" cy="17" r="2.5"/>
            <circle cx="1207" cy="98"  r="3.5"/><circle cx="1214" cy="92"  r="2.5"/>
            <circle cx="1202" cy="166" r="3"/><circle cx="1209" cy="161" r="2.5"/>
            <circle cx="915"  cy="-7"  r="3.5"/><circle cx="907" cy="-13" r="2.5"/>
          </g>
          <g fill="#f5f0e8" stroke="none" opacity="0.22">
            <circle cx="972"  cy="0"   r="2"/><circle cx="824"  cy="47"  r="1.5"/>
            <circle cx="911"  cy="-10" r="1.5"/><circle cx="744" cy="19"  r="1.5"/>
          </g>
        </g>

        {/* Petal pile at bottom */}
        {PILE.map((p,i) => (
      ...
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
        {petals.map(({l,a,d,dl,c,r,s},i) => (
          <div key={i} style={{position:"absolute",top:0,left:`${l}%`,animation:`${a} ${d} ease-in ${dl} infinite`,willChange:"transform,opacity"}}>
...
const DASH_TPLS: Record<string, {bg:string,accent:string,name:string,fee:number,sub:string}> = {
  ...
  Fanfix: {bg:"#0f0a1e",accent:"#a855f7",name:"Fanfix",   fee:0.80,sub:"Supporters"},
  Fanvue: {bg:"#081a18",accent:"#14b8a6",name:"Fanvue",   fee:0.80,sub:"Fans actifs"},
  Reveal: {bg:"#140f00",accent:"#f97316",name:"Inflow",   fee:1.00,sub:"Conversions"},
}
...
export default function Home() {
  ...
  const [gommageProcessing, setGommageProcessing] = useState(false)
  const [gommageError, setGommageError] = useState<string|null>(null)
  const [gommageSlider, setGommageSlider] = useState(50)
  ...
  const [wmSlider, setWmSlider] = useState(50)
  ...
  const generateDashboard = async () => {
    if (!dashAmount || !dashTemplate) return
    setDashProcessing(true); setDashError(null); setDashResult(null)
    try {
      const body: Record<string, string> = { amount: dashAmount, period: dashPeriod, template: dashTemplate }
      if (dashPeriod === 'custom') { body.startDate = dashStartDate; body.endDate = dashEndDate }
      const res = await fetch(`${SERVER_URL}/dashboard/generate`, {
  ...
  const handleDownloadResult = async (dataUrl: string, filename: string) => {
  ...
  const handleGommage = async () => {
    ...
    finally { setGommageProcessing(false) }
  }

  const handleRemoveWatermark = async () => {
    if (!wmFile) return
    setWmProcessing(true); setWmError(null)
    try {
      const fd = new FormData()
      fd.append("image", wmFile)
      const res = await fetch(`${SERVER_URL}/retouch/remove-watermark`, { method:"POST", body:fd })
      const data = await res.json()
      if (data.error) { setWmError(data.error); return }
      setWmResult(data.result); setWmSlider(50)
    } catch (e: any) { setWmError(e.message) }
    finally { setWmProcessing(false) }
  }
  ...
  const handleRemoveText = async () => {
    ...
    } catch (e: any) { setTxtError(e.message) }
    finally { setTxtProcessing(false) }
  }
            ...
            {retoucheTab === "gommage" && (<>
              {gommageResult ? (<>
                ...
                <div style={{ position:"relative", borderRadius:12, overflow:"hidden", userSelect:"none", lineHeight:0, cursor:"col-resize", touchAction:"none" }}
                  ...
                  <div style={{ position:"absolute", top:0, left:0, bottom:0, width:`${gommageSlider}%`, overflow:"hidden", pointerEvents:"none" }}>
                    <img src={gommagePreview!} style={{ width:`${10000/gommageSlider}%`, maxWidth:"none", display:"block" }} draggable={false}/>
                  </div>
                ...
                <label style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:14, padding:"52px 20px", border:`2px dashed rgba(232,245,66,0.2)`, borderRadius:16, background:t.bgCard, cursor:"pointer" }}>
                  <input type="file" accept=".jpg,.jpeg,.png,.webp" style={{ display:"none" }} onChange={e => {
                    ...
                    r.onload = ev => setGommagePreview(ev.target?.result as string)
                    r.readAsDataURL(f)
                  }}/>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <button onClick={() => { setGommageFile(null); setGommagePreview(null); setGommageError(null); clearCanvas() }}
                    style={{ background:"none", border:"none", color:t.textMuted, cursor:"pointer", fontSize:18, lineHeight:1, padding:"0 2px" }}>←</button>
                ...
                {gommageError && (
                  <div style={{ padding:"12px 16px", background:"rgba(255,107,107,0.07)", border:"1px solid rgba(255,107,107,0.2)", borderRadius:10 }}>
                    <p style={{ fontSize:12, color:"#ff6b6b" }}>{gommageError}</p>
                  </div>
                )}
              </>)}
            </>)}
            ...
            {retoucheTab === "watermark" && (<>
              {wmResult ? (<>
                ...
                <div style={{ position:"relative", borderRadius:12, overflow:"hidden", userSelect:"none", lineHeight:0, cursor:"col-resize", touchAction:"none" }}
                  ...
                  <div style={{ position:"absolute", top:0, left:0, bottom:0, width:`${wmSlider}%`, overflow:"hidden", pointerEvents:"none" }}>
                    <img src={wmPreview!} style={{ width:`${10000/wmSlider}%`, maxWidth:"none", display:"block" }} draggable={false}/>
                  </div>
                ...
                <label style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:14, padding:"52px 20px", border:`2px dashed rgba(232,245,66,0.2)`, borderRadius:16, background:t.bgCard, cursor:"pointer" }}>
                  <input type="file" accept=".jpg,.jpeg,.png,.webp" style={{ display:"none" }} onChange={e => {
                    ...
                    const r = new FileReader()
                    r.onload = ev => setWmPreview(ev.target?.result as string)
                    r.readAsDataURL(f)
                  }}/>
                  <span style={{ fontSize:38, opacity:0.5 }}>🖼</span>
                  <div style={{ textAlign:"center" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <button onClick={() => { setWmFile(null); setWmPreview(null); setWmError(null) }}
                    style={{ background:"none", border:"none", color:t.textMuted, cursor:"pointer", fontSize:18, lineHeight:1, padding:"0 2px" }}>←</button>
                  <span style={{ fontSize:13, fontWeight:600, color:t.text, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{wmFile.name}</span>
                </div>
                {wmPreview && (
                  <div style={{ borderRadius:12, overflow:"hidden", lineHeight:0 }}>
                    <img src={wmPreview} style={{ width:"100%", display:"block" }} draggable={false}/>
                  </div>
                )}
                ...
                {wmError && (
                  <div style={{ padding:"12px 16px", background:"rgba(255,107,107,0.07)", border:"1px solid rgba(255,107,107,0.2)", borderRadius:10 }}>
                    <p style={{ fontSize:12, color:"#ff6b6b" }}>{wmError}</p>
                  </div>
                )}
              </>)}
            </>)}
            ...
            {retoucheTab === "text" && (<>
              {txtResult ? (<>
                ...
                <div style={{ position:"relative", borderRadius:12, overflow:"hidden", userSelect:"none", lineHeight:0, cursor:"col-resize", touchAction:"none" }}
                  ...
                  <div style={{ position:"absolute", top:0, left:0, bottom:0, width:`${txtSlider}%`, overflow:"hidden", pointerEvents:"none" }}>
                    <img src={txtPreview!} style={{ width:`${10000/txtSlider}%`, maxWidth:"none", display:"block" }} draggable={false}/>
                  </div>
          ...
          <div style={{ width:"100%", maxWidth:860, padding:"48px 16px 60px", display:"flex", flexDirection:"column", gap:32 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(150px, 1fr))", gap:14 }}>
              {Object.entries(DASH_TPLS).map(([key, tpl]) => (
                <button key={key}
                  onClick={() => { setDashTemplate(key); setDashAmount(""); setDashPeriod("30d"); setDashResult(null); setDashError(null) }}
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", minHeight:"100vh", width:"100%", background:DASH_TPLS[dashTemplate].bg }}>
          <nav style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 16px", borderBottom:`1px solid ${DASH_TPLS[dashTemplate].accent}18`, background:DASH_TPLS[dashTemplate].bg + "ee", backdropFilter:"blur(16px)", WebkitBackdropFilter:"blur(16px)", position:"sticky", top:0, zIndex:50 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={() => { setDashTemplate(null); setDashResult(null); setDashError(null) }}
          ...
          <div style={{ width:"100%", maxWidth:860, padding:"32px 16px 60px", display:"flex", flexDirection:"column", gap:24 }}>
            ...
            <div style={{ background:"rgba(255,255,255,0.04)", border:`1px solid ${DASH_TPLS[dashTemplate].accent}20`, borderRadius:18, padding:"24px 20px", display:"flex", flexDirection:"column", gap:20 }}>
              ...
              <button
                ...
                disabled={!dashAmount || dashProcessing}
                style={{ padding:"14px 0", borderRadius:10, border:"none", background:(dashAmount && !dashProcessing) ? DASH_TPLS[dashTemplate].accent : "rgba(255,255,255,0.06)", color:(dashAmount && !dashProcessing) ? "#0a0a0a" : "rgba(255,255,255,0.25)", cursor:(dashAmount && !dashProcessing) ? "pointer" : "not-allowed", fontSize:14, fontWeight:700, letterSpacing:"0.04em", transition:"background 0.15s" }}>
